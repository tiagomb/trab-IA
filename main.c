...
// read_matrix lê o tabuleiro do jogo da vida.
__always_inline
int* read_matrix(int lin, int col) {
...
__always_inline
void print_solution(Cvc5* slv, Cvc5Term* x, int lin, int col) {
	for (int i = 0; i < lin; i++) {
		for (int j = 0; j < col; j++) {
...
int main(){
	int lin, col;
	if (scanf("%d", &lin) != 1 || scanf("%d", &col) != 1) return 1;

	int *m = read_matrix(lin, col);
	if (!m) return 1;

	// Configuração do CVC5.
